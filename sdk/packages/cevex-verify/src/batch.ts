 ...
 * I/O concurrency but not the theoretical ~3x algebraic speedup. The
 * interface is defined now to allow a drop-in upgrade when the underlying
 * library exposes batch primitives.
 */
...
export async function batchVerifyFalcon(
  ...
  throw new Error('batchVerifyFalcon: FALCON verification is not yet available.')
}
