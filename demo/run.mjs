...
// ─── Box constants ────────────────────────────────────────────────────────────
//
//  Data rows have three columns separated by a vertical divider:
//
//    │  key(K_W)   val(V_W)  │  annotation(A_W)  │
//
//  Total: 1+2+K_W+3+V_W+2 + 1+2+A_W+2+1 = 14+K_W+V_W+A_W = W
//         14 + 12 + 52 + 38 = 116 = W  ✓

const W   = 116
const K_W = 12     // key column visible width
const V_W = 52     // value column visible width
const A_W = 38     // annotation column visible width
const C_W = W - 6  // = 110  banner content width

// Internal divider position (1-indexed): DIV = 1+2+K_W+3+V_W+2+1 = 73
const DIV      = 1 + 2 + K_W + 3 + V_W + 2 + 1   // 73
const L_DASHES = DIV - 2                            // 71
const R_DASHES = W - DIV - 1                        // 42
// Check: 1 + L_DASHES + 1 + R_DASHES + 1 = 1+71+1+42+1 = 116 = W  ✓
...
// ─── Section (single-line with internal divider) ──────────────────────────────

const sTop = (label) => {
  // Visible: ┌─ (3) + label + space (1) + dashes + ┐ (1) = W
  // → dashes = W − 5 − vlen(label)
...
// Three-column data row: key | value | annotation
...
console.log(sEmp())
console.log(sRow('',
  _.dgry + 'github.com/cevexlabs/Cevex  ·  x.com/cevex_io' + _.rst, ''))
console.log(sEmp())
console.log(sBot())
console.log()
